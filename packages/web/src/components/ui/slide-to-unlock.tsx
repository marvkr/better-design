...
function useSlideToUnlock() {
  ...
  if (!context) throw new Error("SlideToUnlock components must be used within SlideToUnlock")
  return context
}
...
export function SlideToUnlock({
  ...
  return (
    <SlideToUnlockContext.Provider
      value={{ x, trackRef, isDragging, handleWidth, textOpacity, onDragStart: handleDragStart, onDragEnd: handleDragEnd }}
    >
      <div
        data-slot="slide-to-unlock"
        className={cn(
          "w-54 rounded-xl bg-secondary p-1 shadow-inner ring-1 ring-border ring-inset",
          className
        )}
        {...props}
      >
...
export function SlideToUnlockHandle({ className, children, style, ...props }: SlideToUnlockHandleProps) {
  ...
  return (
    <motion.div
      data-slot="handle"
      className={cn(
        "absolute top-0 left-0 flex h-10 cursor-grab items-center justify-center rounded-lg bg-card text-muted-foreground shadow-sm active:cursor-grabbing",
    ...
    >
      {children ?? (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" aria-hidden>
          <path d="M237.66,133.66l-96,96A8,8,0,0,1,128,224V184H48a16,16,0,0,1-16-16V88A16,16,0,0,1,48,72h80V32a8,8,0,0,1,13.66-5.66l96,96A8,8,0,0,1,237.66,133.66Z" fill="currentColor" />
        </svg>
      )}
    </motion.div>
  )
