...
// Midnight Glass Multi Select: dropdown with multiple selection
  ...
  return (
    <div ref={ref} className={cn("relative w-full", className)}>
      <button
        ...
        disabled={disabled}
        className={cn(
          "flex min-h-12 w-full flex-wrap items-center gap-1.5 px-5 py-2 text-[15px]",
          ...
          open && "shadow-[inset_0_0_6px_rgba(255,255,255,0.05),0_0_0_3px_rgba(255,255,255,0.2)] border-primary/50",
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full rounded-xl border border-white/[0.08] backdrop-blur-2xl bg-white/[0.07] p-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.6),inset_0_0_6px_rgba(255,255,255,0.05)]">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
              onClick={() => toggle(option.value)}
              className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-[15px] transition-colors hover:bg-white/[0.07]"
            >
              <div
                className={cn(
