...
const toastVariants = cva(
  [
    "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden p-4",
  ...
  ].join(" "),
  {
    variants: {
      variant: {
        default: "bg-white/[0.07] text-foreground",
        destructive:
          "bg-destructive/20 border-destructive/20 text-destructive group",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-full px-3",
      "border border-white/[0.08] bg-white/[0.05]",
      "text-sm font-medium transition-all",
      "hover:bg-white/[0.1]",
      "focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-0",
      "disabled:pointer-events-none disabled:opacity-50",
      "group-[.destructive]:border-destructive/20 group-[.destructive]:hover:border-destructive/30 group-[.destructive]:hover:bg-destructive/20",
      className
    )}
    {...props}
  />
