...
function CodeTabs({ tabs, defaultTab = 0, className }: CodeTabsProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-xl bg-card shadow-md overflow-hidden",
        className
      )}
    >
      {/* Tab bar */}
      <div className="relative flex border-b border-border bg-card">
        {tabs.map((tab, i) => (
          <button
            ...
            className={cn(
              "relative px-4 py-2.5 text-xs font-medium font-mono transition-colors",
              i === activeIndex
                ? "text-foreground"
      ...
      <div className="relative overflow-hidden">
        <AnimatePresence mode="popLayout" initial={false} custom={direction}>
          ...
          >
            <pre className="p-4 m-0 bg-transparent text-xs font-mono leading-relaxed overflow-x-auto text-foreground">
