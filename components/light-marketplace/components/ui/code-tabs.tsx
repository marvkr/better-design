...
function CodeTabs({ tabs, defaultTab = 0, className }: CodeTabsProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-border bg-secondary overflow-hidden",
        className
      )}
    >
      {/* Tab bar */}
      <div className="relative flex border-b border-border bg-secondary/80">
        {tabs.map((tab, i) => (
          <button
            key={tab.label}
      ...
      <div className="relative overflow-hidden">
        <AnimatePresence mode="popLayout" initial={false} custom={direction}>
          ...
          >
            <pre className="p-4 m-0 bg-transparent text-sm font-mono leading-relaxed overflow-x-auto text-foreground">
