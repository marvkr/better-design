...
function Calendar({ className, classNames, showOutsideDays = true, ...props }: CalendarProps) {
 ...
 head_row: "flex",
 head_cell: "text-muted-foreground rounded-none w-8 font-normal text-xs uppercase tracking-wider",
 row: "flex w-full mt-2",
 cell: cn(
 "relative p-0 text-center text-sm focus-within:relative focus-within:z-20",
 "[&:has([aria-selected])]:bg-accent/40 [&:has([aria-selected].day-outside)]:bg-accent/20",
 "first:[&:has([aria-selected])]:rounded-none last:[&:has([aria-selected])]:rounded-none"
 ),
 day: cn(
