...
const MegaMenu = ({ trigger, sections, className }: MegaMenuProps) => {
 ...
 href={item.href ?? "#"}
 className={cn(
 "group flex items-start gap-3 rounded-none p-2",
 "transition-[background-color,box-shadow] duration-150",
 "hover:bg-accent hover:text-accent-foreground"
 )}
 >
 {item.icon && (
 <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-none bg-muted text-primary ">
