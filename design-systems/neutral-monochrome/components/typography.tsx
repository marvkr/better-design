...
// Earthy Typography: dark neutral, near-white primary on near-black
// Geist Sans — clean technical feel, tight headings, relaxed body
// No special tracking — minimal and sharp like the rest of Earthy
...
const H1 = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h1
      ref={ref}
      className={cn(
        "scroll-m-20 text-4xl font-bold tracking-tight text-foreground leading-[1.1]",
        className
      )}
      {...props}
    />
...
const P = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p
      ref={ref}
      className={cn(
        "text-sm text-foreground/80 leading-relaxed [&:not(:first-child)]:mt-4",
        className
      )}
      {...props}
    />
...
const Small = React.forwardRef<HTMLElement, React.HTMLAttributes<HTMLElement>>(
  ({ className, ...props }, ref) => (
    <small
      ref={ref}
      className={cn("text-xs font-medium text-foreground/70 leading-none", className)}
      {...props}
    />
  )
)
...
const Blockquote = React.forwardRef<HTMLQuoteElement, React.HTMLAttributes<HTMLQuoteElement>>(
  ({ className, ...props }, ref) => (
    <blockquote
      ref={ref}
      className={cn(
        "mt-6 border-l-2 border-border pl-5 italic text-muted-foreground text-sm leading-relaxed",
        className
      )}
      {...props}
    />
...
const Code = React.forwardRef<HTMLElement, React.HTMLAttributes<HTMLElement>>(
  ({ className, ...props }, ref) => (
    <code
      ref={ref}
      className={cn(
        "block w-full rounded-lg border border-border bg-secondary px-4 py-3",
        "font-mono text-sm text-foreground/85 leading-relaxed overflow-x-auto",
        className
      )}
      {...props}
    />
...
const InlineCode = React.forwardRef<HTMLElement, React.HTMLAttributes<HTMLElement>>(
  ({ className, ...props }, ref) => (
    <code
      ref={ref}
      className={cn(
        "relative rounded border border-border bg-secondary px-1.5 py-0.5",
