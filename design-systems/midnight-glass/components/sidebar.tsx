...
// Midnight Glass Sidebar: navigation sidebar component
// Deeper glass surface with backdrop-blur, subtle white borders, pill-shaped toggles
  ...
  ({ className, collapsed, children, ...props }, ref) => (
    <div
      ...
      className={cn(
        ...
        "backdrop-blur-xl bg-white/[0.05] border-r border-white/[0.08]",
...
const SidebarHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex h-14 items-center border-b border-white/[0.08] px-4", className)}
      {...props}
    />
  )
)
...
const SidebarFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("border-t border-white/[0.08] p-2", className)}
      {...props}
    />
  )
)
...
const SidebarItem = React.forwardRef<HTMLButtonElement, SidebarItemProps>(
  ({ className, icon, label, active, collapsed, ...props }, ref) => (
    <button
      ref={ref}
      className={cn(
        "flex w-full items-center gap-2.5 rounded-full px-3 py-2 text-sm font-medium transition-all duration-300",
        active
          ? "bg-white/[0.09] text-foreground shadow-[inset_0_0_8px_rgba(255,255,255,0.1)]"
