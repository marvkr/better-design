#!/usr/bin/env node
// Adds the `animate-text` component to every design system in components/<ds>/
// and generates the matching registry/<ds>/animate-text.json. The TSX is
// non-themed and identical across every design system. Variants and timings
...
const REGISTRY_JSON = {
  ...
  files: [
    {
      path: "components/ui/animate-text.tsx",
      content: TSX,
      type: "registry:ui",
    },
  ],
...
for (const ds of designSystems) {
  ...
  await writeFile(join(tsxDir, "animate-text.tsx"), TSX, "utf8")

  await mkdir(jsonDir, { recursive: true })
  await writeFile(
    join(jsonDir, "animate-text.json"),
